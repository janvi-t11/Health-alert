...
export const DataProvider = ({ children }) => {
  ...
  const loadReports = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:4000/api'}/reports`);
  ...
  const value = {
    ...
    rejectReport: async (reportId) => {
      try {
        const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:4000/api'}/reports/${reportId}/reject`, {
          method: 'PUT'
        });
    ...
    deleteReport: async (reportId) => {
      try {
        const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:4000/api'}/reports/${reportId}`, {
          method: 'DELETE'
        });
